Shader "Simple Toon Pro/ST Pro Outline"
{
	Properties
    {
		...
		[Header(Shade)][Space(5)]  //shade
		_MaxAtten ("Shadow Strength", Range(0,1)) = 1
		_PostAtten ("Post Shadow", Range(0,1.1)) = 0
		...
		[HDR] _RimColor ("Color", COLOR) = (0,0,1,1)
		...
		[HDR] _SpcColor ("Color", COLOR) = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag (v2f i) : SV_Target
            {
				...
				fixed4 shadecol = _DarkColor * _ColIntense + _ColBright;
				fixed4 litcol = ColorBlend(_Color, _LightColor0, _AmbientCol);

				fixed4 texcol = tex2D(_MainTex, i.uv) * litcol;
